                        ...
                        <form id="formulario" class="control">
                            <div class="col-md-12 row">
                                <div class="form-group col-md-2">
                                    ...
                                    <select class="form-control" id="anio" name="anio" onchange="Buscar_Mercaderia()">
                                        <option value="0" <?php if (!(strcmp(0, date('Y')))) {echo "selected=\"selected\"";} ?> >Año</option>
                                        <?php foreach($list_anio as $list){ ?>
                                            <option value="<?= $list['cod_anio'] ; ?>" 
                                            <?php if (!(strcmp($list['cod_anio'], date('Y')))) {echo "selected=\"selected\"";} ?> >
                                                <?= $list['cod_anio']; ?>
                                            </option>
                                        <?php } ?>
                                    </select>
                                ...
                                <div class="form-group col-md-2">
                                    ...
                                    <select class="form-control" id="semana" name="semana" onchange="Buscar_Mercaderia()">
                                        <option value="0">Seleccione</option>
                                        <?php $i=1; while($i<=52){ ?>
                                            <option value="<?= $i; ?>" <?php if($i==date('W')){ echo "selected"; } ?>><?= $i; ?></option> 
                                        <?php $i++; } ?>
                                    </select>
                                </div>
                                <div class="form-group col-md-2">
                                    <label class="control-label text-bold">&nbsp;</label>
                                    <button type="button" class="btn btn-primary mb-2 mr-2 form-control" onclick="Cierre_Mercaderia();" >
                                        Cerrar Semana
                                    </button>
                                </div>
                                <div class="form-group col-md-2">
                                    <label class="control-label text-bold">&nbsp;</label>
                                    <a class="btn btn-primary mb-2 mr-2 form-control" style="background-color: #28a745 !important;" onclick="Exportar_MTotal();">
                                        Excel General
...
<script>
    $(document).ready(function() {
        ...
        $("#mercaderia").addClass('active');
        
        Buscar_Mercaderia();
    });
    
    function Buscar_Mercaderia() {
        Cargando();

        var dataString = new FormData(document.getElementById('formulario'));
        var url = "{{ url('MercaderiaExtraer/Buscar_Mercaderia') }}";
    ...
    function Cierre_Mercaderia() {
        Cargando();

        var semana = $('#semana').val();
        var anio = $('#anio').val();
        var url = "{{ url('MercaderiaExtraer/Cierre_Mercaderia') }}";
        var csrfToken = $('input[name="_token"]').val();
        ...
        $.ajax({
            ...
            success: function(resp) {
                if (resp.trim() == "error") {
                    Swal({
                        title: 'Error',
                        text: "La semana " + semana + " ya ha sido cerrada.",
                        type: 'error',
                    ...
                    Swal(
                        'Cerrado',
                        'La semana ' + semana + ' se cerró correctamente.',
                        'success'
                    ).then(function() {
                        Buscar_Mercaderia();
                    });
    ...
    function Exportar_MTotal() {
        var semana = $('#semana').val();
        window.location = "{{ url('MercaderiaExtraer/Excel_MTotal')}}/" + semana;
    }
</script>
