    ...
    <tbody>
        @foreach ($list_cambio_prenda as $list)
            <tr class="text-center">
                ...
                <td>
                    @if (session('usuario')->id_nivel=="1" && $list->estado_cambio!="4" && $list->estado_cambio!="3")
                        <a href="javascript:void(0);" title="Editar" data-toggle="modal" data-target="#ModalUpdate" app_elim="{{ route('cambio_prenda_sin.create') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2 text-success">
                                <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path>
                            </svg>
                        </a>
                        <a href="javascript:void(0);" title="Aprobar" style="cursor:pointer" onclick="Cambiar_Estado_Cambio_Prenda('{{ $list->id_cambio_prenda }}','1')">
                        ...
                        <a href="javascript:void(0);" title="Denegar" style="cursor:pointer" onclick="Cambiar_Estado_Cambio_Prenda('{{ $list->id_cambio_prenda }}','2')">
                    ...
                    @elseif (session('usuario')->id_puesto=="29" || session('usuario')->id_puesto=="30" || session('usuario')->id_puesto=="31" || 
                    session('usuario')->id_puesto=="128" || session('usuario')->id_puesto=="27" || session('usuario')->id_puesto=="197" || 
                    session('usuario')->id_puesto=="161")

                    @endif
                    @if ((session('usuario')->id_nivel=="1" || session('usuario')->id_puesto=="29" || session('usuario')->id_puesto=="32" || 
                    session('usuario')->id_puesto=="31" || session('usuario')->id_puesto=="33" || session('usuario')->id_puesto=="167") && $list->estado_cambio=="2")
