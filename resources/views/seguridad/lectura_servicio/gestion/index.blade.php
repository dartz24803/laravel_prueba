<div class="toolbar d-md-flex align-items-md-center mt-3">
    <div class="form-group col-lg-2">
        ...
        <select class="form-control" id="id_serviciob" name="id_serviciob" onchange="Lista_Gestion_Servicio();">
    ...
    <div class="form-group col-lg-2">
        <label>Base:</label>
        <select class="form-control" id="cod_baseb" name="cod_baseb" onchange="Lista_Gestion_Servicio();">
            <option value="0">TODOS</option>
    ...
    <div class="form-group col-lg-2">
        <label>Mes:</label>
        <select class="form-control" id="mesb" name="mesb" onchange="Lista_Gestion_Servicio();">
            <option value="0">TODOS</option>
    ...
    <div class="form-group col-lg-2">
        <label>Año:</label>
        <select class="form-control" id="aniob" name="aniob" onchange="Lista_Gestion_Servicio();">
            <option value="0">TODOS</option>
    ...
    <div class="col-lg-4">
        <button type="button" class="btn btn-primary mb-2 mb-sm-0 mb-md-2 mb-lg-0" title="Registrar" data-toggle="modal" data-target="#ModalRegistro" app_reg="{{ route('lectura_servicio_ges.create') }}">
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus-square"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="12" y1="8" x2="12" y2="16"></line><line x1="8" y1="12" x2="16" y2="12"></line></svg>
            Registrar
        </button>

        <a class="btn mb-2 mb-sm-0 mb-md-2 mb-lg-0" style="background-color: #28a745 !important;" onclick="Excel_Gestion_Servicio();">
...
@csrf
<div class="table-responsive mb-4 mt-4" id="lista_gestion_servicio">
</div>

<script>
    Lista_Gestion_Servicio();

    function Lista_Gestion_Servicio(){
        Cargando();
        ...
        var mes = $('#mesb').val();
        var anio = $('#aniob').val();
        var url = "{{ route('lectura_servicio_ges.list') }}";
        var csrfToken = $('input[name="_token"]').val();

        $.ajax({
            url: url,
            type: "POST",
            data: {'id_servicio':id_servicio,'cod_base':cod_base,'mes':mes,'anio':anio},
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
            success:function (resp) {
                $('#lista_gestion_servicio').html(resp);  
            }
        });
    }
    ...
    function Traer_Suministro(v){
        ...
        var url = "{{ route('lectura_servicio_ges.traer_suministro') }}";
    ...
    function Traer_Lectura(v){
        ...
        var url = "{{ route('lectura_servicio_ges.traer_lectura') }}";
        var cod_base = $('#cod_base'+v).val();
        var id_servicio = $('#id_servicio'+v).val();
        var id_datos_servicio = $('#id_datos_servicio'+v).val();
        var csrfToken = $('input[name="_token"]').val();

        $.ajax({
            url: url,
            type: "POST",
            data: {'cod_base':cod_base,'id_servicio':id_servicio,'id_datos_servicio':id_datos_servicio},
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
            success:function (resp) {
    ...
    function Descargar_Archivo(id,tipo){
        window.location.replace("{{ route('lectura_servicio_ges.download', [':id', ':tipo']) }}".replace(':id', id).replace(':tipo', tipo));
    }

    function Delete_Gestion_Servicio(id) {
        Cargando();

        var url = "{{ route('lectura_servicio_ges.destroy', ':id') }}".replace(':id', id);
        var csrfToken = $('input[name="_token"]').val();
    ...
    function Excel_Gestion_Servicio() {
        var id_servicio = $('#id_serviciob').val();
        var cod_base = $('#cod_baseb').val();
        var mes = $('#mesb').val();
        var anio = $('#aniob').val();
