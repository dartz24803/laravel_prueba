<div class="toolbar d-md-flex align-items-md-center mt-3">
    ...
    <div class="col-lg-6">
        <button type="button" class="btn btn-primary mb-2 mb-sm-0 mb-md-2 mb-lg-0" title="Registrar" data-toggle="modal" data-target="#ModalRegistro" app_reg="{{ route('lectura_servicio_reg.create') }}">
...
@csrf
<div class="table-responsive mb-4 mt-4" id="lista_lectura_servicio">
</div>
...
<script>
    ...
    function Lista_Lectura_Servicio(){
        ...
        var mes = $('#mesb').val();
        var anio = $('#aniob').val();
        var url = "{{ route('lectura_servicio_reg.list') }}";
        var csrfToken = $('input[name="_token"]').val();

        $.ajax({
            url: url,
            type: "POST",
            data: {'id_servicio':id_servicio,'mes':mes,'anio':anio},
            headers: {
    ...
    function Traer_Suministro(v){
        ...
        var url = "{{ route('lectura_servicio_reg.traer_suministro') }}";
    ...
    function Traer_Lectura(v){
        ...
        var url = "{{ route('lectura_servicio_reg.traer_lectura') }}";
        var id_servicio = $('#id_servicio'+v).val();
        ...
        $.ajax({
            url: url,
            type: "POST",
            data: {'id_servicio':id_servicio,'id_datos_servicio':id_datos_servicio},
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
            success:function (resp) {
    ...
    function Descargar_Archivo(id,tipo){
        window.location.replace("{{ route('lectura_servicio_reg.download', [':id', ':tipo']) }}".replace(':id', id).replace(':tipo', tipo));
    }
