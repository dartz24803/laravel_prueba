...
<div class="d-flex justify-content-end mt-3 w-100">
    <div class="form-group col-lg-2">
        <label>Sede:</label>
        <select class="form-control" id="id_sedeb" name="id_sedeb" onchange="Lista_Control_Camara();">
    ...
    <div class="form-group col-lg-2">
        <label>Local:</label>
        <select class="form-control" id="id_localb" name="id_localb" onchange="Lista_Control_Camara();">
            <option value="0">TODOS</option>
    ...
    <div class="col-lg-8 d-flex align-items-center">
        ...
        <button type="button" class="btn btn-primary" title="Registrar" data-toggle="modal" data-target="#ModalRegistro" app_reg="{{ url('control_camara_reg/modal_ronda') }}">
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus-square">
                <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
                <line x1="12" y1="8" x2="12" y2="16"></line>
                <line x1="8" y1="12" x2="16" y2="12"></line>
        ...
        <a class="btn" style="background-color: #28a745 !important;" onclick="Excel_Control_Camara();">
...
<div class="toggle-switch">
            ...
            <span class="ml-5">Fecha de registro</span>
        </div>

@csrf
<div class="table-responsive mt-4" id="lista_control_carama">
</div>

<script>
    Lista_Control_Camara();

    function Lista_Control_Camara(){
        Cargando();

        var id_sede = $('#id_sedeb').val();
        var id_local = $('#id_localb').val();
        var url = "{{ route('control_camara_reg.list') }}";
        ...
        $.ajax({
            ...
            success:function (data) {
                $("#lista_control_carama").html(data);
            }
        });
    }

    function Excel_Control_Camara() {
        var id_sede = $('#id_sedeb').val();
        var id_local = $('#id_localb').val();
        window.location = "{{ route('control_camara_reg.excel', [':id_sede', ':id_local']) }}".replace(':id_sede', id_sede).replace(':id_local', id_local);
    }
</script>
