<table id="tabla-paginas" class="table table-hover" style="width:100%">
    <thead>
        <tr>
            <th> Orden</th>
    ...
    <tbody>
        <?php foreach ($list as $row) {  ?>
            <tr>
                ...
                <td class="text-center"><?php echo $row['pagina_acceso']; ?></td>
                <td class="text-center">
                <?php if (session('usuario')->centro_labores == "OFC" || session('usuario')->id_puesto == 131 || session('usuario')->id_usuario == 139) { ?>
                    <a href="javascript:void(0);" title="Editar" data-toggle="modal" data-target="#ModalUpdate" app_elim="{{ url('Modal_Update_Pagina/' . $row['id']) }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2 text-success">
                            <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path>
                        </svg>
                    </a>
                    <a href="javascript:void(0);" class="" title="Eliminar" onclick="Delete_Pagina('<?php echo $row['id']; ?>')" id="delete" role="button">
...
<script>
    var tabla = $('#tabla-paginas').DataTable({
