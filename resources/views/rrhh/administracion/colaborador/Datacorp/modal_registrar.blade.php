...
<form id="formulario_insert" method="POST" enctype="multipart/form-data" class="needs-validation">
    <div class="modal-header">
        <h5 class="modal-title">Registrar Acceso Datacorp</h5>
    ...
    <div class="modal-body" style="max-height:450px; overflow:auto;">
        <div class="row">
            ...
            <div class="form-group col-lg-4">
                <select class="form-control basic_i" name="id_area" id="id_area" onchange="Traer_Puesto()">
            ...
            <div class="form-group col-lg-10">
                <input class="form-control" type="text" id="carpeta_acceso" name="carpeta_acceso">
            </div>
        </div>
    </div>
    <div class="modal-footer">
        <button class="btn btn-primary mt-3" onclick="Registrar_Datacorp();" type="button">Guardar</button>
        <button class="btn mt-3" data-dismiss="modal"><i class="flaticon-cancel-12"></i> Cancelar</button>
    </div>
</form>

<script>
    function Registrar_Datacorp() {
        Cargando();
        var csrfToken = $('input[name="_token"]').val();

        var dataString = new FormData(document.getElementById('formulario_insert'));
        var url = "{{ url('Registrar_Datacorp') }}";
...
/*
    function Valida_Registrar() {
        ...
        if ($('#carpeta_acceso').val() == '') {
            Swal(
                'Ups!',
                'Debe ingresar carpeta de acceso.',
                'warning'
            ).then(function() { });
            return false
        }
