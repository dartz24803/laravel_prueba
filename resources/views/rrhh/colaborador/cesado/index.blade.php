...
<!--<div class="toolbar d-md-flex align-items-md-center mt-3">
    <div class="form-group col-lg-4">
        ...
        <div>
            <label class="new-control new-checkbox checkbox-primary">
                <input type="checkbox" id="estado_postulante_1e" class="new-control-input" checked value="1">
                <span class="new-control-indicator"></span>En Proceso
            </label>
            <label class="new-control new-checkbox checkbox-primary">
                <input type="checkbox" id="estado_postulante_2e" class="new-control-input" value="1">
                <span class="new-control-indicator"></span>Seleccionados
            </label>
            <label class="new-control new-checkbox checkbox-primary">
                <input type="checkbox" id="estado_postulante_3e" class="new-control-input" value="1">
    ...
    <div class="form-group col-lg-3 mt-4">
        <label>Área:</label>
        <select class="form-control basic" id="id_areab" name="id_areab" onchange="Lista_Todos();">
            <option value="0">TODOS</option>
        </select>
    </div>

    <div class="col-lg-5">
        <button type="button" class="btn btn-primary mb-2 mb-sm-0 mb-md-2 mb-lg-0" title="Buscar" onclick="Lista_Todos();">
        ...
        <a class="btn mb-2 mb-sm-0 mb-md-2 mb-lg-0" style="background-color: #28a745 !important;" onclick="Excel_Todos();">
...
@csrf
<div class="table-responsive mb-4 mt-4" id="lista_todos">
</div>-->
...
<script>
    ...
    function Lista_Todos(){
        ...
        var estado_postulante_1 = 0;
        var estado_postulante_2 = 0;
        var estado_postulante_3 = 0;
        if ($('#estado_postulante_1e').is(":checked")){
            var estado_postulante_1 = 1;
        }
        if ($('#estado_postulante_2e').is(":checked")){
            var estado_postulante_2 = 1;
        }
        if ($('#estado_postulante_3e').is(":checked")){
            var estado_postulante_3 = 1;
        }
        var id_area = $('#id_areab').val();
        var url = "{{ route('postulante_tod.list') }}";
        var csrfToken = $('input[name="_token"]').val();

        $.ajax({
            url: url,
            type: "POST",
            data: {'estado_postulante_1':estado_postulante_1,'estado_postulante_2':estado_postulante_2,'estado_postulante_3':estado_postulante_3,'id_area':id_area},
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
            success:function (resp) {
                $('#lista_todos').html(resp);  
            }
        });
    }
    ...
    function Excel_Lectura_Servicio() {
        ...
        var mes = $('#mesb').val();
        var anio = $('#aniob').val();
        window.location = "{{ route('lectura_servicio_reg.excel', [':id_servicio', ':mes', ':anio']) }}".replace(':id_servicio', id_servicio).replace(':mes', mes).replace(':anio', anio);
    }*/
</script>