...
<script>
    ...
    function Buscar_Estado_Solicitud_Papeletas_Salida_Seguridad() {
        ...
        var url = "{{ url('Papeletas/Buscar_Base_Papeletas_Seguiridad') }}";
    ...
    function Busca_Colaborador_Control() {
        Cargando();
        var url = "{{ url('Papeletas/Busca_Colaborador_Control') }}";
        var base = $('#base').val();
        var csrfToken = $('input[name="_token"]').val();
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#colaborador_control').html(data);
            }
        });
    }

    function Excel_Estado_Solicitud_Papeletas_Salida_Seguridad() {
        var base = $('#base').val();
        var num_doc = $('#num_doc_control').val();
        var estado_solicitud = $('#estado_solicitud3').val();
        var fecha_revision = $('#fecha_revision3').val().trim();
        var fecha_revision_fin = $('#fecha_revision_fin3').val().trim();
        var url = "{{ url('Papeletas/Buscar_Base_Papeletas_Seguiridad') }}";
        var ini = moment(fecha_revision);
        var fin = moment(fecha_revision_fin);
        ...
        if (ini.isAfter(fin) == true) {
            ...
            msgDate = 'Si va exportar por rango de fechas porfavor ponga la fecha final también  ';
            inputFocus = '#hora_salida_hoy';
            bootbox.alert(msgDate)
            var input = $(inputFocus).parent();
            $(input).addClass("has-error");
            ...
            msgDate = 'Si va exortar por rango de fechas porfavor ponga la fecha inicial también  ';
            inputFocus = '#hora_salida_hoy';
            bootbox.alert(msgDate)
            var input = $(inputFocus).parent();
            $(input).addClass("has-error");
            ...
            window.location = "{{ url('Papeletas/Excel_Estado_Solicitud_Papeletas_Salida_Seguridad') }}/" + base + "/" + estado_solicitud + "/" + fecha_revision + "/" + fecha_revision_fin+"/"+num_doc;
        }
    }
</script>
