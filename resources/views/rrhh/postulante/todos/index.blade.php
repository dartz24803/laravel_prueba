...
<div class="table-responsive" id="lista_todos">
</div>
...
<script>
    ...
    function Retomar_Proceso(id) {
        Cargando();

        var url = "{{ route('postulante_tod.update', ':id') }}".replace(':id', id);

        Swal({
            title: '¿Realmente desea retomar el proceso?',
            text: "El proceso será retomado",
            type: 'warning',
            ...
            if (result.value) {
                $.ajax({
                    ...
                    success: function() {
                        Swal(
                            '¡Retomado!',
                            'El registro ha sido retomado satisfactoriamente.',
                            'success'
                        ).then(function() {
                            Lista_Todos();
                        });
    ...
    function Excel_Todos() {
        ...
        var id_area = $('#id_areab').val();
        window.location = "{{ route('postulante_tod.excel', [':estado', ':id_area']) }}".replace(':estado', estado).replace(':id_area', id_area);
    }
</script>