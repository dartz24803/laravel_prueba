        ...
        <tr>
            ...
            <td>
                <?php if($list['documento']!=""){
                        $image_info = get_headers($url[0]['url_config'].$list['documento']);
                        if (strpos($image_info[0], '200') !== false) {?>
                        <a style="cursor:pointer;display: -webkit-inline-box;" data-title="DNI vista" data-toggle="modal" data-target="#Modal_IMG_Link" data-imagen="<?php echo $url[0]['url_config'].$list['documento']?>">
            ...
            <td>
                <a style="cursor:pointer;" title="Eliminar" onclick="Delete_Hijos_Usuario('<?php echo $list['id_hijos']; ?>','<?php echo $list['id_usuario']; ?>')" id="delete" role="button">
