<div class="row mr-1 ml-1 mt-2">
    <div class="form-group col-lg-2">
        <label>Base:</label>
        <select class="form-control" id="baseb" name="baseb" onchange="Lista_Mercaderia_Nueva();">
...
<script>
    ...
    function Lista_Mercaderia_Nueva(){
        ...
        var url = "{{ route('requerimiento_tienda_mn.list') }}";
    ...
    function Excel_Mercaderia_Nueva() {
        ...
        var anio = $('#aniob').val();
        var semana = $('#semanab').val();
        window.location = "{{ route('requerimiento_tienda_mn.excel', [':base', ':anio', ':semana']) }}".replace(':base', base).replace(':anio', anio).replace(':semana', semana);
    }
</script>