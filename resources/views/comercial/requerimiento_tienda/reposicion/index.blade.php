<div class="row mr-1 ml-1 mt-2">
    <div class="form-group col-lg-2">
        <label>Base:</label>
        <select class="form-control" id="baseb" name="baseb" onchange="Lista_Requerimiento_Reposicion();">
    ...
    <div class="form-group col-lg-2">
        <label>Año:</label>
        <select class="form-control" id="aniob" name="aniob" onchange="Lista_Requerimiento_Reposicion();">
            <option value="0">Todos</option>
    ...
    <div class="form-group col-lg-2">
        <label>Semana:</label>
        <select class="form-control" id="semanab" name="semanab" onchange="Lista_Requerimiento_Reposicion();">
            <option value="0">Todos</option>
    ...
    <div class="col-lg-6 d-flex align-items-center">
        <a class="btn" style="background-color: #28a745 !important;" onclick="Excel_Requerimiento_Reposicion();">
...
<div class="table-responsive" id="lista_requerimiento_reposicion">
</div>

<script>
    Lista_Requerimiento_Reposicion();

    function Lista_Requerimiento_Reposicion(){
        Cargando();

        var base = $('#baseb').val();
        var anio = $('#aniob').val();
        var semana = $('#semanab').val();
        var url = "{{ route('requerimiento_tienda_re.list') }}";
        ...
        $.ajax({
            ...
            success:function (resp) {
                $('#lista_requerimiento_reposicion').html(resp);  
            }
        });
    }

    function Excel_Requerimiento_Reposicion() {
        var base = $('#baseb').val();
        var anio = $('#aniob').val();
        var semana = $('#semanab').val();
        window.location = "{{ route('requerimiento_tienda_re.excel', [':base', ':anio', ':semana']) }}".replace(':base', base).replace(':anio', anio).replace(':semana', semana);
    }
</script>