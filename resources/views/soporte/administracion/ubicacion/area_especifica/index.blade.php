<div class="toolbar d-flex">
    <div class="col-lg-12 d-flex justify-content-end">
        <button type="button" class="btn btn-primary" title="Registrar" data-toggle="modal" data-target="#ModalRegistro"
            app_reg="{{ route('soporte_area_esp_conf.create') }}">
...
<script>
    ...
    function Lista_AreaEspecifica() {
        Cargando();

        var url = "{{ route('soporte_area_esp_conf.list') }}";
    ...
    function Delete_AreaEspecifica(id) {
        Cargando();

        var url = "{{ route('soporte_area_esp_conf.destroy', ':id') }}".replace(':id', id);
