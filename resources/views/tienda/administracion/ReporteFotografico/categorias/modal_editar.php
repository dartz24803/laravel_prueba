...
<form id="formulario_update" method="POST" enctype="multipart/form-data" class="needs-validation">
    ...
    <div class="modal-body row" style="max-height:450px; overflow:auto;">
        <div class="form-group col-md-6">
            <label for="my-input">Categoria : <span class="text-danger">*</span></label>
            <input type="text" class="form-control" name="codigo_e" id="codigo_e" value="<?= $get_id[0]['categoria'] ?>">
        </div>
        <div class="form-group col-md-6">
            <label>Area: </label>
            <select class="form-control basic_i" id="area_e" name="area_e[]" multiple>
                <?php foreach($list_area as $list){ ?>
                    <option value="<?php echo $list['id_area']; ?>"
                    <?php 
                    foreach($get_id2 as $row){
                        if($list['id_area'] == $row['id_area']){ 
                            echo "selected"; 
                        }; 
                    }
                    ?>>
