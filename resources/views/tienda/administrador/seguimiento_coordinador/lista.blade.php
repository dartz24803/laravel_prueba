<table id="tabla_js" class="table" style="width:100%">
    ...
    <tbody>
        @foreach ($list_seguimiento_coordinador as $list)
            <tr class="text-center">
                <td>{{ $list->orden }}</td>
                <td>{{ $list->base }}</td>
                <td>{{ $list->fecha }}</td>
                ...
                <td>
                    @if ($list->v_evidencia>0)
                        ...
                        app_elim="{{ route('administrador_sc.evidencia', $list->id) }}">
                ...
                <td class="text-center">
                    <div class="btn-group dropleft" role="group"> 
                        ...
                        <div class="dropdown-menu" aria-labelledby="btnDropLeft" style="padding:0;">
                            ...
                            data-target="#ModalUpdate" 
                            app_elim="{{ route('administrador_sc.show', $list->id) }}">
                                Ver
                            </a>
                            @if (session('usuario')->id_puesto!=311)
                                @if (date('Y-m-d')==$list->orden)
                                    ...
                                    app_upd_grande="{{ route('administrador_sc.edit', $list->id) }}">
                                        Editar
                                    </a>

                                    <a href="javascript:void(0);" class="dropdown-item" 
                                    onclick="Delete_Seguimiento_Coordinador('{{ $list->id }}')">
                                        Eliminar
                                    </a>
                                @endif
                            @endif
