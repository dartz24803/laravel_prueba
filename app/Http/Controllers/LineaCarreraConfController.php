...
class LineaCarreraConfController extends Controller
{
    ...
    public function edit_pre($id)
    {
        ...
        return view('caja.administracion.linea_carrera.pregunta.modal_editar', compact('get_id'));
    }
    ...
    public function update_pre(Request $request, $id)
    {
        ...
        $valida = Pregunta::where('nom_tipo_error', $request->nom_tipo_errore)->where('estado', 1)->where('id_tipo_error', '!=', $id)->exists();
        if($valida){
            echo "error";
        }else{
    ...
    public function list_reva()
    {
        ...
        return view('caja.administracion.linea_carrera.revision_evaluacion.lista', compact('list_error'));
    }*/
}
