...
class RequerimientoSurtidoController extends Controller
{
    ...
    public function Insert_Requerimiento()
    {
        ...
        for ($indiceFila = 2; $indiceFila <= $numeroMayorDeFila; $indiceFila++) {
            ...
            $dato['B21'] = $hojaDeProductos->getCell('AN' . $indiceFila)->getValue();

            # Continúa con BEC y RN
            $dato['BEC'] = $hojaDeProductos->getCell('AO' . $indiceFila)->getValue();
            $dato['RN'] = strtoupper($hojaDeProductos->getCell('AP' . $indiceFila)->getValue());

            if($dato['empresa']=="" && $dato['costo']=="" && $dato['pc']=="" && $dato['pv']=="" &&
            ...
            $dato['color']=="" && $dato['talla']=="" && $dato['stock']=="" && $dato['B01']=="" &&
            $dato['B02']=="" && $dato['B03']=="" && $dato['B04']=="" && $dato['B05']=="" &&
            $dato['B06']=="" && $dato['B07']=="" && $dato['B08']=="" && $dato['B09']=="" &&
            $dato['B10']=="" && $dato['B11']=="" && $dato['B12']=="" && $dato['B13']=="" &&
            $dato['B14']=="" && $dato['B15']=="" && $dato['B16']=="" && $dato['B17']=="" &&
            $dato['B18']=="" && $dato['B19']=="" && $dato['B20']=="" && $dato['B21']=="" &&
            $dato['BEC']=="" && $dato['RN']==""){
                break;
            }
            ...
            if (
                ...
                substr($dato['B18'], 0, 1) != "=" && substr($dato['B21'], 0, 1) != "=" &&
                substr($dato['RN'], 0, 1) != ""   && strlen($dato['RN']) < 2
            ) {
                $total = $total + intval(strval($dato['B01'])) + intval(strval($dato['B02'])) +
                        ...
                        intval(strval($dato['B11'])) + intval(strval($dato['B13'])) +
                        intval(strval($dato['B12'])) + intval(strval($dato['B14'])) +
                        intval(strval($dato['B15'])) + intval(strval($dato['BEC'])) +
                        intval(strval($dato['B16'])) + intval(strval($dato['B17'])) +
                        intval(strval($dato['B18'])) + intval(strval($dato['B19'])) +
                        intval(strval($dato['B20'])) + intval(strval($dato['B21']));
                ...
                if ((trim($dato['RN']) !== 'N' && trim($dato['RN']) !== 'R' && trim($dato['RN']) !== 'T') || $list_duplicados) {
